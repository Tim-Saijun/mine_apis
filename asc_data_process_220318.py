...
def generate_csv(names:list, target_dir=''):
    
    csv_name=names[0]+"_n"+str(len(names))+".csv"
...
def process(source_dir,target_dir,rtype):
    ...
    if rtype.lower()=='df':
        ...
        return generate_csv(names=save_fname_list,target_dir=target_dir)
    else:
        raise ValueError("Parameter 'rtype' must be 'csv' or 'df'")

if __name__ == '__main__':
    source_dir = r"C:\Users\Administrator\Desktop\Shared\2020-12-01 12.58.41 263.W.asc"
    target_dir = r"C:\Users\Administrator\Desktop"
    process(source_dir,target_dir,rtype='csv')
